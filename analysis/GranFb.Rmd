...
DEGenesFbGrancond2 <- FindAllMarkers(seuratFbGran, only.pos=T) %>% 
  dplyr::filter(p_val_adj < 0.01) 

write.table(DEGenesFbGrancond2, 
            file= "/Users/immbio/Desktop/Project/Angelina/Cxcl13Hpb/data/DEGenesFbGrancond2.txt",
            sep="\t",
            quote=F,
            row.names=F,
            col.names=T)
...
#top 100 DE genes Glial
seurat1 <- subset(seuratFb, celltype == "Glial")
table(seurat1$cond2)
levels(seurat1) 

DEGenesGlial <- FindAllMarkers(seurat1, logfc.threshold = 0.1, only.pos = T) %>% 
  dplyr::filter(p_val_adj < 0.01) %>% mutate(celltype = "Glial")

DEGenesGlial_100 <- DEGenesGlial %>% top_n(100, avg_log2FC) 

DEGenesAll100 <- dplyr::bind_rows(DEGenesPdgfraloFb_100, DEGenesTrophocytes_100, DEGenesTelocytes_100, DEGenesMyocytes_100, DEGenesPdgfrahi_100, DEGenesThy1Fb_100, DEGenesGlial_100)
```
...
#top 100 DE genes Glial
seurat1 <- subset(seuratFbGran, celltype == "Glial")
table(seurat1$cond2)
levels(seurat1) 
...
DEGenesGlial_100 <- DEGenesGlial %>% top_n(100, avg_log2FC) 
DEGenesGlial_50 <- DEGenesGlial %>% top_n(50, avg_log2FC) 

DEGenesAll100 <- dplyr::bind_rows(DEGenesPdgfraloFb_100, DEGenesTrophocytes_100, DEGenesTelocytes_100, DEGenesMyocytes_100, DEGenesPdgfrahi_100, DEGenesThy1Fb_100, DEGenesGlial_100)

DEGenesAll50 <- dplyr::bind_rows(DEGenesPdgfraloFb_50, DEGenesTrophocytes_50, DEGenesTelocytes_50, DEGenesMyocytes_50, DEGenesPdgfrahi_50, DEGenesThy1Fb_50, DEGenesGlial_50)
```
...
#top 100 DE genes Glial
seurat1 <- subset(seuratFb, clustername == "Glial")
table(seurat1$cond2)
levels(seurat1) 

DEGenesGlial <- FindAllMarkers(seurat1, logfc.threshold = 0.1, only.pos = T) %>% 
  dplyr::filter(p_val_adj < 0.01) %>% mutate(clustername = "Glial")

DEGenesGlial_100 <- DEGenesGlial %>% top_n(100, avg_log2FC) 

DEGenesAll100 <- dplyr::bind_rows(DEGenesPdgfraloFb1_100, DEGenesPdgfraloFb2_100, DEGenesPdgfraloFb3_100,DEGenesPdgfraloFb4_100, DEGenesTrophocytes_100, DEGenesTelocytes_100, DEGenesMyocytes_100, DEGenesPdgfrahi_100, DEGenesThy1Fb_100, DEGenesGlial_100)
```
...
## exclude Gial cells
seuratFbGranwt <- subset(seuratFbGranwt, clustername == "Glial", invert =TRUE)
table(seuratFbGran$cond2)
table(seuratFbGranwt$cond2)
table(seuratFbGranwt$clustername)
...
### save seurat object
saveRDS(seuratFbGran, file="/Users/immbio/Desktop/Project/Angelina/Cxcl13Hpb/data/merged_GranFb_seurat.rds")
```

## umaps 
```{r load and umaps FbGran}
##load object Fb gran rerun
fileNam <- "/Users/immbio/Desktop/Project/Angelina/Cxcl13Hpb/data/merged_GranFb_seurat.rds"
